#!/usr/bin/env node
/**
 * Final test - try to fill the 60x30mm label
 */
...
async function printLabel(widthBytes, heightLines, description) {
  ...
  for (let y = 0; y < heightLines; y++) {
    for (let x = 0; x < widthBytes; x++) {
      // Thick border (4 bytes)
      if (y < 4 || y >= heightLines - 4 || x < 4 || x >= widthBytes - 4) {
        data[y * widthBytes + x] = 0xFF;
      }
      // Diagonal line from top-left to bottom-right
      else if (Math.abs(y * widthBytes / heightLines - x) < 3) {
        data[y * widthBytes + x] = 0xFF;
      }
      // Center dot
      else if (Math.abs(y - heightLines/2) < 5 && Math.abs(x - widthBytes/2) < 5) {
        data[y * widthBytes + x] = 0xFF;
      }
      else {
...
async function main() {
  ...
  try {
    ...
    await printLabel(72, 350, 'TEST 1: 72 x 350 lines');
    await delay(2000);

    await printLabel(72, 400, 'TEST 2: 72 x 400 lines');
    await delay(1000);

    console.log('\n' + '='.repeat(50));
    console.log('Which test fills the label better?');
    console.log('Look for the diagonal line from corner to corner.');
